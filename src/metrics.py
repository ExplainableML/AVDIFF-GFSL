...
class MeanClassAccuracy(Metric):
    def __init__(self, model, dataset, device, distance_fn,new_model_sequence, args, use_calibration, which):
        ...
        self.both_beta = []
        self.both_zsl=[]
        self.both_discriminator_seen = []
        self.both_discriminator_unseen = []

    def __call__(self, outputs, target, loss_outputs, dictionary):

        evaluation = evaluate_dataset_baseline(dataset_tuple=self.dataset, model=self.model, device=self.device,
                                               dictionary=dictionary,
                                               args=self.args, which=self.which)
        ...
        self.both_discriminator_seen.append(evaluation["both"]['correct_seen'])
        self.both_discriminator_unseen.append(evaluation['both']['correct_unseen'])
    ...
    def value(self):
        return {
            ...
            "both_discriminator_seen": np.mean(self.both_discriminator_seen),
            "both_discriminator_unseen": np.mean(self.both_discriminator_unseen)
        }
